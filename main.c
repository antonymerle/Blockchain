...
int main(void)
{
	...
	// uint8_t const merkle_proof[12][HEX_HASH_NT_SZ - 1] = {
	...
	//uint8_t* str = concat(txid, merkle_proof);

	//uint8_t mr[SHA256_DIGEST_LENGTH] = { 0 };
	//uint8_t mrhex[HEX_HASH_NT_SZ] = { 0 };

	//digest_hash_str(mr, str);
	//digest_bin_2_hex(mrhex, HEX_HASH_NT_SZ, mr, SHA256_DIGEST_LENGTH);
